...
module.exports = {
    ...
    componentDidMount: function() {
        if (this.getDOMNode().addEventListener) {
        ...
        if (this.state.useHistory) {
            if (window.addEventListener) {
                ...
                window.attachEvent('onpopstate', this.onPopState);
            }
        } else {
    ...
    componentWillUnmount: function() {
        if (this.getDOMNode().removeEventListener) {
            this.getDOMNode().removeEventListener('click', this.handleClick);
        } else {
            this.getDOMNode().detachEvent('onclick', this.handleClick);
        }
        ...
        if (this.state.useHistory) {
            if (window.removeEventListener) {
                ...
                window.detachEvent('onpopstate', this.onPopState);
            }
        } else {
            if (window.removeEventListener()) {
...
function getInitialPath(component) {
    ...
    if (!path && detect.canUseDOM) {
        ...
        if (component.props.history) {
            ...
            var hash = urllite(url.hash.slice(2));
